...
export const LocationsEdit = () => {
    ...
    const recordBriefs: RecordBriefModel[] = useSelector(
        (state: { recordBriefs: RecordBriefModel[] }) => state.recordBriefs
    );
    ...
    const currentLocationGroup: LocationGroup = locationGroups.find(
        (l) => l.id === locationGroupInEditId
    ) as LocationGroup;
    const allRawLocations: LocationModel[] =
    ...
    const allLocations: LocationWithUses[] = allRawLocations.map((l) => {
        const usesRoutes = routeStopsWithLocations
            .filter((s) => s.locations.includes(l.id))
            .map((s) => s.name);
        const usesTransports = transportsWithLocations
            .filter((s) => s.locations.includes(l.id))
            .map((s) => s.name);
                    ...
                    {filteredLocations.map((l: LocationWithUses, i: number) => (
                        ...
                        >
                            <LocationForm
                                ...
                                isInEdit={locationMainInEdit === l.id}
                                {...l}
                                isLocationNameValid={l.IsValid}
