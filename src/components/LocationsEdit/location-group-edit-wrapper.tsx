...
export const LocationGroupEditWrapper = () => {
    ...
    const locationGroupInEditId = useSelector(
        (state: SidurStore) =>
            state.sessionState.locationGroupInEdit || "ESHBAL"
    );
    const locationGroups: LocationGroup[] = useSelector(
        (state: { LocationGroups: LocationGroup[] }) =>
            state.LocationGroups || []
    );
    const recordBriefs: RecordBriefModel[] = useSelector(
        (state: { recordBriefs: RecordBriefModel[] }) =>
            state.recordBriefs || []
    );
    ...
    const handleLocationGroupMenuOpen = (
    ...
    const buildTextForLocationInSidurim = (
        sidurim: RecordBriefModel[]
    ): string => {
        let retText = "";
        if (sidurim.length === 0) {
            return "";
        ...
        if (sidurim.length >= 4) {
            retText +=
                ", " +
                translations.thisCollectionIsInUseandAnother +
                " " +
                (sidurim.length - 1).toString() +
                " " +
                translations.thisCollectionIsInUseandAnothersidurim;
        }
        retText += ". " + translations.thisCollectionIsInUsedCannotbeDeleted;
        return retText;
    };

    const isLocationInSidur: RecordBriefModel[] | [] = recordBriefs.filter(
        (lb) =>
            lb.typeOfRecord === TypeOfRecord.Sidur &&
            lb.locationGroupOrSidurId === locationGroupInEditId
    );

    const handleLocationGroupMenuClick = (
        event: React.MouseEvent<HTMLElement>,
    ...
    const handleLocationGroupChanged = (
        event: any,
    ...
    const locationGroupInEdit: LocationGroup =
        locationGroups.find(
            (lGroup: LocationGroup) => lGroup.id === locationGroupInEditId
        ) || (locationGroups[0] as LocationGroup);
    ...
    return (
        <div>
                ...
                <div className="flex flex-row items-center justify-start mb-[10px]">
                    ...
                    >
                        ...
                        <MenuItem key={"NEW"} value={"NEW"}>
                            {" "}
                            &nbsp; <b>
                                {translations.CreateLocationGroup}
                            </b>{" "}
                            &nbsp;
                        </MenuItem>
                        {locationGroups
                            .filter((l) => l.id !== "ESHBAL")
                            .map((oneLocationGroup: LocationGroup) => (
                                <MenuItem
                                    key={oneLocationGroup.id}
                                    value={oneLocationGroup.id}
                                >
                                    {" "}
                                    {oneLocationGroup.name} &nbsp;{" "}
                                </MenuItem>
                            ))}
                    </Select>
            ...
            {locationGroupInEditId === "ESHBAL" ? (
                <LocationCantEditMessage />
            ) : (
                <LocationsEditTabs />
            )}
            <LocationGroupMenu
                locationGroupMoreAnchorEl={locationGroupMoreAnchorEl}
                ...
                preventDelete={isLocationInSidur.length > 0}
            />
            <RenameDialog
                open={RenameOpen}
