...
export const SketchesContainerMessage = (props: {
    sketch: SketchModel | null;
    clickCancel: () => void;
}) => {
    ...
    const pendingOrderInEditAction = useSelector(
        (state: SidurStore) => state.sessionState.pendingOrderInEditAction
    );
    const pendingOrderIdInEdit = useSelector(
        (state: SidurStore) => state.sessionState.pendingOrderIdInEdit
    );

    let messageText = "";
    if (pendingOrderInEditAction) {
        const pendingORderInEdit: OrderModel | undefined =
            props.sketch?.unassignedOrders.find(
                (o) => o.id === pendingOrderIdInEdit
            );
        ...
        if (pendingORderInEdit) {
            messageText +=
                " " +
                LanguageUtilities.buildBriefText(pendingORderInEdit, locations)
                    .driverAndLocation;
        }
        if (
