...
export const LocationsTransportEditWrapper = () => {
    ...
    const transportRouteIdInEdit = useSelector(
        (state: SidurStore) => state.sessionState.transportIdInEdit
    );
    ...
    const allLocationsFromStore: LocationModel[] =
        currentLocationGroup?.Locations || [];
    const allTransportRoutes: TransportModel[] =
    ...
    const transportRoutInEdit: TransportModel | undefined =
        currentLocationGroup.Transports.find(
            (r) => r.id === transportRouteIdInEdit
        );
    ...
    const routInEditLocations: string[] = transportRoutInEdit
        ? transportRoutInEdit.TransportStops.map((rs) => rs.locationId)
        : [];
    ...
    const filteredRoutes =
        filterRouteText.trim() === ""
            ? allTransportRoutes.filter((l) => l)
            : allTransportRoutes.filter((l) =>
                  l.name.includes(filterRouteText.trim())
              );
    ...
    return (
        <div className={Styles.flexRow}>
            ...
            <div className={`${Styles.flexColumn} m-[1em] mt-0`}>
                ...
                {transportRoutInEdit ? (
                    <LocationTransportEdit
                        allLocations={allLocations}
                        transportRoute={transportRoutInEdit}
                    />
                ) : null}
            </div>
            <div className={Styles.flexColumn}>
                    ...
                    <div dir="rtl" id={"routes-container"}>
                        {filteredRoutes.map((r: TransportModel) => (
                            <LocationRouteTransportChoose
                                routeOrTransport={RouteOrTransport.Transport}
                                key={r.id}
                                route={r}
