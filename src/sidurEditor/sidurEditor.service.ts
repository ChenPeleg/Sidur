...
export const SidurEditorService = {
    getRelevantDriveIdsToChoose(
        sketch: SketchModel,
        pendingOrderId: string
    ): string[] {
        const pendingOrder = sketch.unassignedOrders.find(
    ...
    getOnlyOneRelevantDriveIdsToChoose(
        sketch: SketchModel,
        pendingOrderId: string
    ): string[] {
        const pendingOrder = sketch.unassignedOrders.find(
    ...
    shouldIMergeDrives(
        origin: { start: number; finish: number },
        destination: { start: number; finish: number }
    ): boolean {
    ...
    SuggestMergedDrive(
        drive: DriveModel,
        orderToMerge: OrderModel,
        locations: LocationModel[]
    ): { suggestedDrive: DriveModel; issues: string[] } {
        ...
        const startGapInMinutes = Utils.HourDecimalToMinutes(
            Math.abs(driveStart - orderStart)
        );
        const finishGapInMinutes = Utils.HourDecimalToMinutes(
            Math.abs(driveFinish - orderFinish)
        );
        ...
        if (
            ...
            let message =
                ...
                " " +
                startGapInMinutes.toString() +
                " " +
                translations.gapForMergeDrives3StartTimes;
            issues.push(message);
        }
        if (
            ...
            let message =
                translations.gapForMergeDrives1 +
                " " +
                finishGapInMinutes.toString() +
                " " +
                translations.gapForMergeDrives3FinishTimes;
            issues.push(message);
        }
    ...
    splitTsamudOrder(
        ...
        const location = locations.find(
            (l: LocationModel) => l.id === order.location
        );
        const eta = location?.ETA || 30;
        const orderTo = CloneUtil.deepCloneOrder(order);
        const orderFrom = CloneUtil.deepCloneOrder(order);

        orderTo.finishHour = Utils.DecimalTimeToHourText(
            Utils.hourTextToDecimal(orderTo.startHour) +
                Math.floor(((eta + 5) * 2) / 60)
        );
        orderTo.TypeOfDrive = DriveType.OneWayTo;
        orderTo.Comments = translations.beforeSplit + ": " + orderTo.Comments;

        orderFrom.startHour = Utils.DecimalTimeToHourText(
            Utils.hourTextToDecimal(orderFrom.finishHour) -
                Math.floor(((eta + 5) * 2) / 60)
        );
