...
const DetectFormRowsWithOnlyYear = (completeText: string): string => {
    return completeText.replace(
        /\n((0?[1-9]|[12][0-9]|3[01])[/-](0?[1-9]|1[012])[/-]\d{2})/g,
        NewRowToken + "$1"
    );
};
 ...
 */
const searchAnotherTimeInText = (
    order: OrderModel
): { anotherTime: string | null } => {
...
const searchLocationInText = (
    ...
    if (results.locationFound) {
        ...
        const fromPrefixes = [
            translations.From,
            translations.fromLocationWithThe,
        ];
...
const getLocationAndTypeFromComments = (
    orders: OrderModel[],
    locations: LocationModel[]
): OrderModel[] => {
    orders.map((order) => {
        const LocationSearchResult = searchLocationInText(
            order.Comments,
            locations
        );
        ...
        if (
            anotherTimeSearchResults.anotherTime &&
            anotherTimeSearchResults.anotherTime !== order.startHour
        ) {
...
export const ImportOrdersFromText = (
    text: string,
    locations: LocationModel[]
): OrderModel[] => {
    const isNewDateFormat = !DetectIfRowsHas6figuersDatesInThem(text);
    const rowsWithoutUserLineBreaks = isNewDateFormat
        ? DetectFormRowsWithOnlyYear(text)
        : DetectFormRowsWithFullDate(text);
...
export const validateImportedData = (orderPreferences: OrderModel[]) => {
    ...
    const driverWithoutHours = orderPreferences.filter((p) => !p.startHour);
    ...
    if (driverWithoutHours[0]) {
        errors.push(
            "Driver " + driverWithoutHours[0].driverName + " has no dates"
        );
    }
    ...
    if (driverWithError[0]) {
        errors.push(
            "Row " +
                orderPreferences.indexOf(driverWithError[0]) +
                " has an error"
        );
    }
    if (errors.length) {
