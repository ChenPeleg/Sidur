...
export class CloneUtil {
    ...
    static deepCloneVehicleSchedules(
        obj: VehicleScheduleModel
    ): VehicleScheduleModel {
    ...
    static deepCloneSidur(sidur: SidurRecord): SidurRecord {
        ...
        clonedSidur.orders = clonedSidur.orders.map((o) =>
            CloneUtil.deepCloneOrder(o)
        );
        clonedSidur.deletedOrders = clonedSidur.deletedOrders.map((o) =>
            CloneUtil.deepCloneOrder(o)
        );
        clonedSidur.sketches = clonedSidur.sketches.map((o) =>
            CloneUtil.deepCloneSketch(o)
        );
        clonedSidur.defaultOrderValues = clonedSidur.defaultOrderValues
            ? { ...clonedSidur.defaultOrderValues }
            : undefined;
        return clonedSidur;
    }

    static deepCloneSketch(obj: SketchModel): SketchModel {
        obj.vehicleSchedules = obj.vehicleSchedules.map((vs) =>
            CloneUtil.deepCloneVehicleSchedules(vs)
        );
        return { ...obj };
    }
    ...
    public static deep(obj: any, name: string): any {
        ...
        switch (name) {
            ...
            case "OrderModel":
                newObj.flexibility = [
                    newObj.flexibility[0],
                    newObj.flexibility[1],
                ];
        }
        return newObj;
    }
