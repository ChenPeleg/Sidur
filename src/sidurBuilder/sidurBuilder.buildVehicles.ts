...
export const SidurBuilderBuildVehiclesAndUnAssigned = (
    orders: OrderMetaDataModel[],
    ...
    const metaOrderScheduleData: OrdMetaScheduleData[] = orders.map(
        (o: OrderMetaDataModel) => {
            return {
                start: o.start,
                finish: o.finish,
                id: o.id,
    ...
    const vehicleSchedules: VehicleScheduleModel[] = vehicles.map(
    ...
    vehicleSchedules.forEach((schedule: VehicleScheduleModel) => {
        ...
        metaOrderScheduleData.forEach((metaOrder) => {
            if (metaOrder.start > currentHour) {
                const relevantMetaDrive: OrderMetaDataModel | undefined =
                    orders.find((meta) => meta.id === metaOrder.id);
                if (
                    !relevantMetaDrive ||
                ...
                newDrive.description = LanguageUtilities.buildBriefText(
                    fakeOrder,
                    locations
                ).driverAndLocation;
    ...
    const unassignedOrders: OrderModel[] = orders
        .filter((o) => o.status === OrderMetaStatus.None)
        .map((o) => o.order);

    const assignedOrders: OrderModel[] = orders
        .filter((o) => o.status !== OrderMetaStatus.None)
        .map((o) => o.order);
    return {
        vehicleSchedules,
