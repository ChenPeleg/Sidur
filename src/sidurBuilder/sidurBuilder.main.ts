...
export const SidurBuilder = (
    Sidur: SidurRecord,
    locations: LocationModel[],
    _buildSettings: any = null
): SketchModel => {
    const settings: SidurBuildSettings = { custom: null };
    const ordersMetaData: OrderMetaDataModel[] =
        SidurBuilderBuildOrdersMetaData(Sidur.orders, locations, settings);
    const BuildResult = SidurBuilderBuildVehiclesAndUnAssigned(
        ordersMetaData,
        Sidur.vehicles,
        locations,
        settings
    );
    const initialVehicles: VehicleScheduleModel[] =
        BuildResult.vehicleSchedules;
