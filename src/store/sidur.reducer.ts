...
> = {
    [ActionsTypes.CHOOSE_SIDUR]: (
        ...
        const chosenSidurObj: SidurRecord | undefined =
            newState.sidurCollection.find(
                (record: SidurRecord) => record.id === chosenSidurId
            );
        if (chosenSidurObj !== undefined) {
            const previousSidurObj: SidurRecord | undefined =
                newState.sidurCollection.find(
                    (record: SidurRecord) => record.id === previousSidurId
                );
            if (previousSidurObj !== undefined) {
                ...
                NewPreviousSidurObj.deletedOrders = newState.deletedOrders.map(
                    (o) => ({
                        ...o,
                    })
                );
    ...
    [ActionsTypes.DELETE_SIDUR]: (
        ...
        if (sidurIdToDelete.includes(AppConstants.ArchiveIdPrefix)) {
            newState.sidurArchive = newState.sidurArchive.map((sidur) => {
                if (sidurIdToDelete === sidur.id) {
                    const updatedSidur = { ...sidur };
                    updatedSidur.id = updatedSidur.id.replace(
                        AppConstants.ArchiveIdPrefix,
                        AppConstants.deleteIdPrefix
                    );
                    return updatedSidur;
                }
                return sidur;
            ...
            let deletedSidur: SidurRecord | undefined =
                newState.sidurCollection.find((s) => s.id === sidurIdToDelete);
            if (deletedSidur) {
                deletedSidur = { ...deletedSidur };
                deletedSidur.id = "Del" + deletedSidur.id;
                newState.sidurArchive.push(deletedSidur);
            }
            newState.sidurCollection = newState.sidurCollection.filter(
                (s) => s.id !== sidurIdToDelete
            );
        }
        ...
        if (newState.sidurId === sidurIdToDelete) {
            const chosenSidurAfterDelete: SidurRecord =
                newState.sidurCollection[0];
    ...
    [ActionsTypes.ARCHIVE_SIDUR]: (
        ...
        if (sidurIdToArchive.includes(AppConstants.deleteIdPrefix)) {
            ...
            let archivedSidur: SidurRecord | undefined =
                newState.sidurCollection.find((s) => s.id === sidurIdToArchive);
            if (archivedSidur) {
                archivedSidur = { ...archivedSidur };
                archivedSidur.id =
                    AppConstants.ArchiveIdPrefix + archivedSidur.id;
                newState.sidurArchive.push(archivedSidur);
                newState.sidurCollection = newState.sidurCollection.filter(
                    (s) => s.id !== sidurIdToArchive
                );
            }
        }
        ...
        if (newState.sidurId === sidurIdToArchive) {
            const chosenSidurAfterArchive: SidurRecord =
                newState.sidurCollection[0];
    ...
    [ActionsTypes.CLONE_SIDUR]: (
        ...
        let sidurForCloning: SidurRecord | undefined =
            newState.sidurCollection.find((s) => s.id === sidurIdToClone);
    ...
    [ActionsTypes.MOVE_TO_ACTIVE_SIDUR]: (
        ...
        let toActivateSidur: SidurRecord | undefined =
            newState.sidurArchive.find((s) => s.id === sidurIdToActivate);
        if (toActivateSidur) {
            ...
            newState.sidurArchive = newState.sidurArchive.filter(
                (s) => s.id !== sidurIdToActivate
            );
        }
