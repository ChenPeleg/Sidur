...
> = {
    ...
    [ActionsTypes.CLONE_LOCATION_GROUP]: (
        ...
        let locationGroupForCloning: LocationGroup | undefined =
            newState.LocationGroups?.find(
                (s) => s.id === locationGroupIdToClone
            );

        if (locationGroupForCloning) {
            const newLocGroup: LocationGroup = CloneUtil.deepCloneLocationGroup(
                locationGroupForCloning
            );
            newLocGroup.name = translations.CopyOf + " " + newLocGroup.name;
            const newLocGroupId = Utils.getNextId(
                newState.LocationGroups?.map((l) => l.id) || [""]
            );
            newLocGroup.id = newLocGroupId;
            newLocGroup.dbId = "";
            newState.LocationGroups =
                newState.LocationGroups?.map((c) => c) || [];
