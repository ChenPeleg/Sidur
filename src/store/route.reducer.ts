...
export const RouteReducer: Record<
    RouteReducerFunctions,
...
> = {
    ...
    [ActionsTypes.ADD_LOCATION_TO_ROUTE]: (
        ...
        if (currentLocationGroup && locationToAdd) {
            ...
            const routeInEdit: RouteModel | undefined =
                currentLocationGroup.Routes.find((r) => r.id === routeIdInEdit);
            if (routeInEdit) {
                ...
                currentLocationGroup.Routes = currentLocationGroup.Routes.map(
                    (r) => (r.id === routeInEdit.id ? routeInEdit : r)
                );
    ...
    [ActionsTypes.START_EDIT_ROUTE]: (
        ...
        if (currentLocationGroup) {
            const route = currentLocationGroup.Routes.find(
                (l) => l.id === routId
            );
            if (route) {
    ...
    [ActionsTypes.UPDATE_ROUTE]: (
        ...
        if (currentLocationGroup && routeToUpdate) {
            const routBeforeUpdate = currentLocationGroup.Routes.find(
                (r) => r.id === routeToUpdate.id
            ) as RouteModel;
            ...
            if (
                isNameAutoBuild &&
                routBeforeUpdate.name === routeToUpdate.name
            ) {
                routeToUpdate.name = buildRouteName(
                    routeToUpdate,
                    currentLocationGroup.Locations
                );
            }
    ...
    [ActionsTypes.DELETE_ROUTE]: (
        ...
        if (currentLocationGroup) {
            currentLocationGroup.Routes = currentLocationGroup.Routes.filter(
                (l) => l.id !== routeToDeleteId
            );
    ...
    [ActionsTypes.CLONE_ROUTE]: (
        ...
        if (currentLocationGroup) {
            const routeToClone: RouteModel | undefined =
                currentLocationGroup.Routes.find(
                    (r) => r.id === routeToCloneId
                );
