...
> = {
    [ActionsTypes.NEW_VEHICLE]: (
        ...
        if (existingNames.includes(newVehicle.vehicleName)) {
            newVehicle.vehicleName =
                newVehicle.vehicleName + " " + newVehicle.id.toString();
        }
    ...
    [ActionsTypes.UPDATE_VEHICLE]: (
        ...
        if (updateVehicle) {
            ...
            const existingNames = state.vehicles
            ...
            if (existingNames.includes(updateVehicle.vehicleName)) {
                updateVehicle.vehicleName =
                    updateVehicle.vehicleName +
                    " " +
                    updateVehicle.id.toString();
            }
            const newIdIfNeeded = Utils.getNextId(
    ...
    [ActionsTypes.DELETE_VEHICLE]: (
        ...
        newState.vehicles = newState.vehicles.filter(
            (vehicle) => deleteVehiclerId !== vehicle.id
        );
