...
export const LocationReducer: Record<
    LocationReducerFunctions,
...
> = {
    [ActionsTypes.ADD_NEW_LOCATION]: (
        ...
        if (currentLocationGroup) {
            const newId = Utils.getNextId(
                currentLocationGroup.Locations.map((l) => l.id)
            );
            ...
            const currentSidur = newState.sidurCollection.find(
                (s) => s.id === newState.sidurId
            ) as SidurRecord;
            if (
                currentLocationGroupId === currentSidur.locationGroupIdForSidur
            ) {
    ...
    [ActionsTypes.START_EDIT_LOCATION]: (
        ...
        if (currentLocationGroup) {
            const location = currentLocationGroup.Locations.find(
                (l) => l.id === locationId
            );
            if (location) {
                newState.sessionState.locationMainInEdit = locationId;
    ...
    [ActionsTypes.UPDATE_LOCATION]: (
        ...
        if (currentLocationGroup && locationToUpdate) {
            currentLocationGroup.Locations = currentLocationGroup.Locations.map(
                (l) => {
    ...
    [ActionsTypes.DELETE_LOCATION]: (
        ...
        if (currentLocationGroup) {
            currentLocationGroup.Locations =
                currentLocationGroup.Locations.filter(
                    (l) => l.id !== locationToDeleteId
                );
