...
export const OrderReducer: Record<
    OrderReducerFunctions,
...
> = {
    [ActionsTypes.ADD_NEW_ORDER]: (
        ...
        newState.sidurCollection =
            StoreUtils.UpdateSidurCollectionWithCurrenSidur(newState);
    ...
    [ActionsTypes.CLONE_ORDER]: (
        ...
        const cloneOriginOrder = newState.orders.find(
            (order) => cloneOriginId === order.id
        );
        if (cloneOriginOrder) {
    ...
    [ActionsTypes.UPDATE_ORDER]: (
        ...
        newState.orders = newState.orders.map((order) => {
            if (
                orderId === order.id &&
                newState.sessionState.dataHolderForCurrentOrderInEdit
            ) {
    ...
    [ActionsTypes.DELETE_ORDER]: (
        ...
        newState.orders = newState.orders.filter(
            (order) => deleteOrderId !== order.id
        );
        if (
            newState.sessionState.dataHolderForCurrentOrderInEdit &&
            newState.sessionState.dataHolderForCurrentOrderInEdit.id ===
                deleteOrderId
        ) {
            newState.sessionState.dataHolderForCurrentOrderInEdit = null;
        }
...
const updateOrdersWithEditedOrder = (state: SidurStore): SidurStore => {
    const currentOrderId =
        state?.sessionState?.dataHolderForCurrentOrderInEdit?.id;
    if (currentOrderId) {
        state.orders = state.orders.map((order) => {
            if (
                currentOrderId === order.id &&
                state.sessionState.dataHolderForCurrentOrderInEdit
            ) {
