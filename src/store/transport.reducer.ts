...
export const TransportReducer: Record<
    TransportReducerFunctions,
...
> = {
    [ActionsTypes.ADD_NEW_TRANSPORT]: (
        ...
        let newState = { ...state };
        const currentLocationGroupId =
            newState.sessionState.locationGroupInEdit;
        const currentLocationGroup: LocationGroup | undefined =
            newState.LocationGroups?.find(
                (l) => l.id === currentLocationGroupId
            );

        if (currentLocationGroup) {
            currentLocationGroup.Transports =
                currentLocationGroup.Transports || [];
            const newId = Utils.getNextId(
                currentLocationGroup.Transports.map((l) => l.id)
            );
            ...
            newTransport.name = buildTransportName(
                newTransport,
            ...
            newState.LocationGroups = (newState.LocationGroups || []).map(
                (g) => {
    ...
    [ActionsTypes.ADD_LOCATION_TO_TRANSPORT]: (
        ...
        if (currentLocationGroup && locationToAdd) {
            ...
            const transportInEdit: TransportModel | undefined =
                currentLocationGroup.Transports.find(
                    (r) => r.id === transportIdInEdit
                );
            if (transportInEdit) {
                const isNameAutoBuild =
                    transportInEdit.name ===
                    buildTransportName(
                        transportInEdit,
                        currentLocationGroup.Locations
                    );
                const allPositions = transportInEdit.TransportStops.map(
                    (s) => s.position
                );
                allPositions.push(0);
                ...
                if (isNameAutoBuild) {
                    transportInEdit.name = buildTransportName(
                        transportInEdit,
                        currentLocationGroup.Locations
                    );
                }
                currentLocationGroup.Transports =
                    currentLocationGroup.Transports.map((r) =>
                        r.id === transportInEdit.id ? transportInEdit : r
                    );
    ...
    [ActionsTypes.START_EDIT_TRANSPORT]: (
        ...
        if (currentLocationGroup) {
            const transport = currentLocationGroup.Transports.find(
                (l) => l.id === transportId
            );
            if (transport) {
    ...
    [ActionsTypes.UPDATE_TRANSPORT]: (
        ...
        if (currentLocationGroup && transportToUpdate) {
            const routBeforeUpdate = currentLocationGroup.Transports.find(
                (r) => r.id === transportToUpdate.id
            ) as TransportModel;

            const isNameAutoBuild =
                routBeforeUpdate.name ===
                buildTransportName(
                    routBeforeUpdate,
                    currentLocationGroup.Locations
                );
            ...
            if (
                isNameAutoBuild &&
                routBeforeUpdate.name === transportToUpdate.name
            ) {
                transportToUpdate.name = buildTransportName(
                    transportToUpdate,
                    currentLocationGroup.Locations
                );
            }
            ...
            newState.LocationGroups = newState.LocationGroups.map((lg) =>
                lg.id === currentLocationGroup.id ? currentLocationGroup : lg
            );
        }
    ...
    [ActionsTypes.DELETE_TRANSPORT]: (
        ...
        if (currentLocationGroup) {
            currentLocationGroup.Transports =
                currentLocationGroup.Transports.filter(
                    (l) => l.id !== transportToDeleteId
                );
            newState.LocationGroups = newState.LocationGroups.map((l) =>
                l.id === currentLocationGroup.id ? currentLocationGroup : l
            );
        }
    ...
    [ActionsTypes.CLONE_TRANSPORT]: (
        ...
        if (currentLocationGroup) {
            const transportToClone: TransportModel | undefined =
                currentLocationGroup.Transports.find(
                    (r) => r.id === transportToCloneId
                );
            ...
            if (transportToClone) {
                ...
                newTransport.TransportStops = newTransport.TransportStops.map(
                    (rs) => ({ ...rs })
                );
                newTransport.name = buildTransportName(
                    newTransport,
...
const buildTransportName = (
    ...
    if (transport.TransportStops.length === 0) {
        ...
        return (
            translations.newTransport +
            " " +
            transport.id.toString() +
            " - " +
            getLocationName(transport.TransportStops[0].locationId, locations)
        );
    } else if (
        transport.TransportStops.length > 1 &&
        transport.TransportStops.length < 4
    ) {
        return transport.TransportStops.map((s) =>
            getLocationName(s.locationId, locations)
        ).join(" - ");
    } else {
        return transport.TransportStops.map((s) =>
            getLocationName(s.locationId, locations)
            ...
            .filter(
                (l, i) =>
                    i === 0 ||
                    i + 2 > transport.TransportStops.length ||
                    Math.floor(transport.TransportStops.length / 2) === i
            )
            .join(" - ");
    }
};
